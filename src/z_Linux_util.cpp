...
void node_create(int graph_id,char label[])
{
FILE *fp1;
 fp1 = fopen("graph.graphml", "a+");
    fprintf(fp1,"<data key=\"d0\"/>\n");                    //start node creation
fprintf(fp1,"<node id=\"%d\">\n",graph_id);
...
extern void edge_create(char source[], char dest[]); 
...
char thr_id[10];
    KA_TRACE( 10, ("__kmp_create_worker: done creating thread (%d)\n", gtid ) );
    	int gtid1=gtid;
	char label[10];
	sprintf(label, "Thread : %d", gtid);
	node_create(gtid,label);
	sprintf(thr_id,"%d",gtid1);
	edge_create("START", thr_id);
//---------------------------------------------------------------------------------------------------------------------------------------------------------
} // __kmp_create_worker
