...
__kmp_for_static_init(
    ...
    #ifdef KMP_DEBUG
    {
        ...
        buff = __kmp_str_format(
            ...
            traits_t< T >::spec, traits_t< T >::spec, traits_t< ST >::spec, traits_t< T >::spec );
        KD_TRACE(100, ( buff, *plastiter, *plower, *pupper, *pstride ) );
    //    printf("\n--iter: %d--lower: %d-- upper: %d-- stride: %d",*plastiter, *plower, *pupper, *pstride );
	__kmp_str_free( &buff );
    }
    #endif
    KE_TRACE( 10, ("__kmpc_for_static_init: T#%d return\n", global_tid ) );
...
void node_create(char graph_id[],char label[])
{
FILE *fp1;  
 fp1 = fopen("graph.graphml", "a+");
    fprintf(fp1,"<data key=\"d0\"/>\n");                    //start node creation
fprintf(fp1,"<node id=\"%s\">\n",graph_id);
fprintf(fp1,"<data key=\"d5\"/>\n");
fprintf(fp1,"<data key=\"d6\">\n");
fprintf(fp1,"<y:ShapeNode>\n");
fprintf(fp1,"<y:Geometry height=\"62.0\" width=\"63.0\" x=\"389.0\" y=\"149.0\"/>\n");
fprintf(fp1,"<y:Fill color=\"#FFCC00\" transparent=\"false\"/>\n");
fprintf(fp1,"<y:BorderStyle color=\"#000000\" raised=\"false\" type=\"line\" width=\"1.0\"/>\n");
fprintf(fp1,"<y:NodeLabel alignment=\"center\" autoSizePolicy=\"content\" fontFamily=\"Dialog\" fontSize=\"12\" fontStyle=\"plain\"");
fprintf(fp1," hasBackgroundColor=\"false\" hasLineColor=\"false\" height=\"17.96875\" horizontalTextPosition=\"center\" iconTextGap=\"4\"");
fprintf(fp1," modelName=\"custom\" textColor=\"#000000\" verticalTextPosition=\"bottom\" visible=\"true\" width=\"57.671875\"");
fprintf(fp1," x=\"2.6640625\" y=\"22.015625\"> %s <y:LabelModel>\n",label);
fprintf(fp1,"<y:SmartNodeLabelModel distance=\"4.0\"/>\n");
fprintf(fp1,"</y:LabelModel>\n");
fprintf(fp1,"<y:ModelParameter>\n");
fprintf(fp1,"<y:SmartNodeLabelModelParameter labelRatioX=\"0.0\" labelRatioY=\"0.0\" nodeRatioX=\"0.0\"");
fprintf(fp1," nodeRatioY=\"0.0\" offsetX=\"0.0\" offsetY=\"0.0\" upX=\"0.0\" upY=\"-1.0\"/>\n");
fprintf(fp1,"</y:ModelParameter>\n");
fprintf(fp1,"</y:NodeLabel>\n");
fprintf(fp1,"<y:Shape type=\"ellipse\"/>\n");
fprintf(fp1,"</y:ShapeNode>\n");
fprintf(fp1,"</data>\n");
fprintf(fp1,"</node>\n");
...
void edge_create(int dest, int source)
{

FILE *fp1;
 fp1 = fopen("graph.graphml", "a+");
        fprintf(fp1,"\n<edge id=\"%d%d\" source=\"%d\" target=\"%d\">\n",source,dest,source,dest);  //edge from start to first thread of the program
        fprintf(fp1,"<data key=\"d10\"/>\n");
        fprintf(fp1,"<data key=\"d11\">\n");
        fprintf(fp1,"<y:PolyLineEdge>\n");
        fprintf(fp1,"<y:Path sx=\"15.75\" sy=\"26.84375\" tx=\"0.0\" ty=\"-31.0\">\n");
        fprintf(fp1,"<y:Point x=\"208.16646825396825\" y=\"77.5\"/>\n");
        fprintf(fp1,"<y:Point x=\"293.6246031746032\" y=\"77.5\"/>\n");
        fprintf(fp1,"</y:Path>\n");
...
for(i=0;i<counter;i++)
{
printf("\ngtid: %d",thread_map[0][i]);
printf("\nlower: %d",thread_map[1][i]);
printf("\nupper: %d",thread_map[2][i]);
printf("\nchunk: %d",thread_map[3][i]);
...
//printf("\n\nid: %d--lower:%d--upper:%d--chunk:%d ...kmpc_for_static_init\n\n",gtid,*plower,*pupper,chunk);
char label[30]=" ";
char lower1[10],upper1[10],chunk1[10],graph_id[10];
snprintf(lower1, sizeof(lower1), "lower:%d", *plower);
snprintf(upper1, sizeof(upper1), "upper:%d", *pupper);
snprintf(chunk1, sizeof(chunk1), "chunk:%d", chunk);
//snprintf(graph_id, sizeof(graph_id), "%d_%d", counter,gtid);
snprintf(graph_id, sizeof(graph_id), "%d_%d", *plower,*pupper);
strcat(label,lower1);
strcat(label,upper1);
strcat(label,chunk1);

	thread_map[0][counter]=gtid;
	thread_map[1][counter]=*plower;
	thread_map[2][counter]=*pupper;
	thread_map[3][counter]=chunk;
counter++;
node_create(graph_id,label);
//print_thread_map();
