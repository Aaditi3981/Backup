...
extern void node_create(char graph_id[],char label[]);
...
{
printf("\n %d",thread_map[0][i]);
printf("\t %d",thread_map[1][i]);
printf("\t %d",thread_map[2][i]);
printf("\t %d",thread_map[3][i]);
...
char *start = "START";
char *zero = "0";
node_create(start,start);
node_create(zero,"Thread 0");
int i;
char source[10],dest[10];
for(i=0;i<counter;i++)
{
	sprintf(source, "%d", thread_map[0][i]);
	sprintf(dest, "%d_%d", thread_map[1][i],thread_map[2][i]);
	//edge_create(thread_map[0][i],thread_map[0][i]);
	edge_create(source,dest);

}
edge_create(start,zero);
omp_yed_end();
